...
void CParamManager::LoadConfiguration()
{
	...
	// m_pInputFile
	::GetPrivateProfileStringA( "SPB2BP", "Input", "stdin", m_pInputFile, 1024, m_pIniName );
	
	// m_pOutputDir
	::GetPrivateProfileStringA( "SPB2BP", "OutputDir", "C:\\Lidar\\", m_pOutputDir, 1024, m_pIniName );

	// m_bClip, m_dbClip
	::GetPrivateProfileStringA( "SPB2BP", "Clip", "", pString, MAX_STRING, m_pIniName );
...
void CParamManager::RegisterCommandLine( int argc, char *argv[] )
{
	...
	if ( argc > 2 ) {
		sprintf_s( m_pOutputDir, 1024, "%s", argv[ 2 ] );
	}
}
