// SPB2LAS.cpp : Defines the entry point for the console application.
//
...
void main( int argc, char * argv[] )
{
	...
	while ( bLoop ) {
		if ( reader.ReadNextElement() ) {		// read chunk information
			...
			SPBPoint * point = reader.GetPoint();
			num++;
			...
			if ( temp > mark ) {
					...
					&& ( point->pos[1] <= manager->m_dbClip[1][1] ) ) )
				{
					...
					case PT_Building:
						...
						pt.SetIntensity( point->patch % 10 );
						break;
					case PT_Tree:
						pt.SetClassification( 4 );
						break;
					case PT_Ground:
						pt.SetClassification( 2 );
						break;
					default:
