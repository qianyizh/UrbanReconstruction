...
void CParamManager::LoadConfiguration()
{
	...
	// m_pInputList
	::GetPrivateProfileStringA( "SPB2LAS", "Input", "stdin", m_pInputFile, 1024, m_pIniName );
	
	// m_pOutputFile
	::GetPrivateProfileStringA( "SPB2LAS", "Output", "stdout", m_pOutputFile, 1024, m_pIniName );

	// m_bClip, m_dbClip
	::GetPrivateProfileStringA( "SPB2LAS", "Clip", "", pString, MAX_STRING, m_pIniName );
	...
	// m_dbSampleRate
	::GetPrivateProfileStringA( "SPB2LAS", "SampleRate", "0.99", pString, MAX_STRING, m_pIniName );
	m_dbSampleRate = atof( pString );

	// m_bIntensity
	m_bIntensity = ( bool )::GetPrivateProfileIntA( "SPB2LAS", "PatchToIntensity", 1, m_pIniName );
