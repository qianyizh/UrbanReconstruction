...
void CMCGrid::HermiteData_SaveToTxt( char filename[] )
{
	FILE * f;
		...
		if ( nodes.size() > 0 ) {
			for ( int i = 0; i < m_nWidth - 1; i++ ) {
				for ( int j = 0; j < m_nHeight - 1; j++ ) {
					MCGridNode & node = nodes[ ( i << m_nLevel ) + j ];
...
//					MCGridNode & node = m_vecNodes[ l ][ ( i >> l << ( m_nLevel - l ) ) + ( j >> l) ];
//					if ( node.pass ) {
...
void CMCGrid::PrintMatrix( float **a, int matrix_size )
{
...
void CMCGrid::PrintVector( float *w, int vector_size )
{
